...
Reminds users by automatically sending a pop message inside APP before 2 hours of events beginning time.
...
****
* In UniLA,remind function is already set up by default. Every event will be reminded 2 hours before event. `remind`
  can be used to change the default set up.
...
* `REMINDTIME_BEFORE` must be a positive number 1,2,...,15,16,...,60,... Unit is MIN
* `REMINDTIME_AFTER` refers to how long would user prefer to wait before next repeated reminder.
* `REMINDTIME_AFTER` must be a positive number 1,2,...,15,16,...,60,... Unit is MIN
