...
=== Deleteing a reminder : `deleteE `
Delete reminders related to specific event. Or delete specific reminder.
Format: `deleteE e/EVENT_LIST_INDEX r/REMINDER_LIST_INDEX`

****
* Can only use `deleteE e/EVENT_LIST_INDEX` or `deleteE r/REMINDER_LIST_INDEX`.
* If users input `deleteE e/EVENT_LIST_INDEX r/REMINDER_LIST_INDEX`, UniLA only consider `e/EVENT_LIST_INDEX`.
* `EVENT_LIST_INDEX` refers to index number shown in the displayed Event List
* `EVENT_LIST_INDEX` must be a positive integer 1,2,3,4,...
