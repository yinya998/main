...
The mechanism is facilitated by `findCommand`, `findCommandParser` and different Predicate classes. Searching can be done with 3 different search patterns. During the execution of find command, 3 search patterns will be executed one by one.
...
[NOTE]
Different search patterns optimize the 'find' command for different kinds of users. They help with users who cannot remember the exact spelling or who are prone to typos.
...
The mechanism is facilitated by `Photo`, `PhotoCommand` class. It extends `Modle` with a `Photo` class. It allows the user to add a photo to the person in the contact list.
