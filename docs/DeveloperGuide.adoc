...
image::MeetCommandLogic6a.png[width="1000"]

image::MeetCommandLogic7.png[width="1000"]

Here, the event in blue clashes with the event in green. Just as before, the two old events is reduced to a new meeting event that starts right after the end of `y` and is within the block.

image::MeetCommandLogic8.png[width="1000"]

Finally, the last event does not clash with the event in blue. Both events reduce to the event in blue.

image::MeetCommandLogic9.png[width="1000"]

This event is then tested whether its end `DateTime` is later than the latest end `DateTime` specified by the user, and whether there are duplicate events.
...
image::ReminderUseCase.png[width="800"]
...
.. Prerequisites: List all events using the `listE` command. Multiple events in the list. List all reminders using `listFr` command. Multiple reminders are in the right panel.
.. Test case: `addR 11 t/2 u/min` +
   Expected: For 11th event, we added a reminder which will pop up 2 minutes before 11th event's starting time. In the right panel, the list will add this reminder in the end.
.. Test case: `addR 11 t/3 u/mIN` +
   Expected: For 11th event, we added a reminder which will pop up 3 minutes before 11th event's starting time. In the right panel, the list will add this reminder in the end.
.. Test case: `addR 11 t/4 u/houR` +
   Expected: For 11th event, we added a reminder which will pop up 3 hours before 11th event's starting time. In the right panel, the list will add this reminder in the end.
.. Test case: `addR 11 t/4 u/hour` +
...
.. Test case: `addR 10 t/x u/min`(where x is the interval between reminder pop up time and 10th event start time) +
    `listR`
   Expected: `x` mins before the 10th event beginning, under `listR` view, there will be a pop up reminder in the right reminder list panel.
   After showing up for around one minute, the reminder will be deleted automatically.

=== Delete a reminder
..Test case: `deleteR e/6`
  Expected: All reminders related to 6th event will be deleted. These reminders will not show under both `listFr` and `listR`.
