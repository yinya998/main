...
The Event list component complements Contact list to provide user with better manage events and their participants. To achieve this, a new event model is created to according to the below class diagram.
...
** Pros: Easy to implement.
** Cons: `AddressBook` need to manage both contacts and events, thus have to implement many functions.
...
** Cons: This will need much more implementation to build the event list component

// end::eventlistcomponent-jwl1997[]

// tag::contacteventconnection-jwl1997[]
=== Connection between contact and event
==== Current Implementation
...
To achieve this function and other contact-event related functions, a unidirectional reference is maintained from `Event` to `Person`, as shown below.
...
** Pros: Easier to implement, sufficient for all the operations and less memory/storage required
...
** Pros: Still easy to implement, easier to get the events that each contact has.
** Cons: Connect and Disconnect need to change both two referencing lists, more storage needed.
...
* MeetCommand retrieves persons from AddressBook. The persons retrieved will be stored temporarily in the instance.
* For each person, retrieve list of events within the next week. These events will be used to find available timeslots.
* Find a timeslot(s) which everyone can attend.
* If no timeslot(s) are available, prompt that there are no common timeslots among the people listed.
* Otherwise, get blocks of timeslot(s) which the given people can meet.
* Show list of possible meeting timeslots and the best location to meet.
* Once user chooses the desired timeslot, this meet event will be added to all users listed.
...
The `findE` command is supported by many predicate classed, which implements Predicate Interface. Each field has its own set of predicates.

The following diagram shows the composition of the `AllPredicate` class and the relationship between `FindResults`:

image::AllPredicates.png[width="600"]
Composition of `AllPredicate` class
